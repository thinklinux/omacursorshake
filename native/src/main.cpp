...
void* pmf_address(T pmf) {
    ...
    if (representation.ptr & 0x01) {
        Log::logger->log(Log::ERR, "[omacursorshake] aborting hook on virtual function pointer");
...
CFunctionHook* hook(void* target, std::string signature, void* handler) {
    Log::logger->log(Log::INFO, "[omacursorshake] starting to hook for {} at {:p}", signature, target);

    Dl_info info = {};
    if (!dladdr(target, &info)) {
        Log::logger->log(Log::ERR, "[omacursorshake] aborting hook without associated symbol");
...
// this makes sure that the function signature has not changed without us noticing
    ...
    if (signature != info.dli_sname) {
        Log::logger->log(Log::ERR, "[omacursorshake] aborting hook, function symbol changed to {}", info.dli_sname);
        throw std::runtime_error("unexpected function signature, are you up-to-date?");
    }
#else
    Log::logger->log(Log::INFO, "[omacursorshake] unchecked symbol compiled against unknown libc++ is {}", info.dli_sname);
    Log::logger->log(Log::WARN, "[omacursorshake] hooking on unknown libc++, signatures are not checked, this might crash");
#endif

    auto hook = HyprlandAPI::createFunctionHook(PHANDLE, target, handler);

    Log::logger->log(Log::INFO, "[omacursorshake] checks passed, actually hooking");
    if (!hook->hook()) {
        Log::logger->log(Log::ERR, "[omacursorshake] could not hook, hooking failed");
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    if (COMPOSITOR_HASH != CLIENT_HASH) {
        HyprlandAPI::addNotification(PHANDLE, "[omacursorshake] failed to load, version mismatch!", CHyprColor{1.0, 0.2, 0.2, 1.0}, 10000);
    ...
    // CHighresHandler::update() bails while g_pDynamicCursors is still null
    // during construction. Load the magnified hyprcursor theme now that the
    // pointer exists, so shake zoom is not stuck on a nearest-neighbour
    // upscale of the 24px shape until the next config reload.
    g_pDynamicCursors->updateTheme();
    ...
    try {
        ...
        Log::logger->log(Log::ERR, "[omacursorshake] failed to hook, {}", e.what());
        HyprlandAPI::addNotification(PHANDLE, std::format("[omacursorshake] cannot load, {}", e.what()), CHyprColor{1.0, 0.2, 0.2, 1.0}, 5000);
        throw e;
    } catch (...) {
        Log::logger->log(Log::ERR, "[omacursorshake] failed to hook for unknown reason");
        HyprlandAPI::addNotification(PHANDLE, "[omacursorshake] cannot load, unknown error with hooks!", CHyprColor{1.0, 0.2, 0.2, 1.0}, 5000);
        throw std::runtime_error("hooks failed for unknown reason");
    }

    return {"omacursorshake", "shake the mouse to enlarge the cursor", "Virt / Tihomir Valkanov", "1.0"};
}
